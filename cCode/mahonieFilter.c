...
// #define twoKiDef	(2.0f * 1000.0f)	// 2 * integral gain
...
static float integralFBx = 1.0f,  integralFBy = 1.0f, integralFBz = 1.0f; // integral error terms scaled by Ki
...
void fusion_read(float *retQ0, float *retQ1, float *retQ2, float *retQ3)
{
	*retQ0 = q0;
	*retQ1 = q1;
	*retQ2 = q2;
...
//gyro in rad/sec, accelerometer in g
void mahony_updateIMU(float gx, float gy, float gz, float ax, float ay, float az)
{
	float recipNorm;
...
void mahony_update_full(float gx, float gy, float gz,
